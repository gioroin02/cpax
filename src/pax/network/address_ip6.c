...
pxAddressIp6FromString(PxAddressIp6* self, PxString8 string)
{
    ...
    switch (pxString8ContainsMemory(string, pxCast(pxword8*, "::"), 2)) {
        case 0: {
            pxint groups = pxString8ContainsMemory(string, pxCast(pxword8*, ":"), 1);

            if (groups != PX_ADDRESS_IP6_GROUPS - 1) return 0;

            for (pxint i = 0; i < PX_ADDRESS_IP6_GROUPS; i += 1) {
                pxString8SplitMemory(string, pxCast(pxword8*, "."), 1, &group, &string);
        ...
        case 1: {
            pxString8SplitMemory(string, pxCast(pxword8*, "::"), 2, &left, &right);

            pxint pivot = pxString8ContainsMemory(left, pxCast(pxword8*, ":"), 1) + 1;
            pxint start = 0;
            ...
            for (pxint i = start; i < pivot; i += 1) {
                pxString8SplitMemory(left, pxCast(pxword8*, ":"), 1, &group, &left);
            ...
            start = pivot;
            pivot = pxString8ContainsMemory(right, pxCast(pxword8*, ":"), 1) + 1;

            if (start + pivot >= stop) return 0;

            for (pxint i = stop - pivot; i < stop; i += 1) {
                pxString8SplitMemory(right, pxCast(pxword8*, ":"), 1, &group, &right);
