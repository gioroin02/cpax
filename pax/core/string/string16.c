...
pxString16CopyUnicode(PxArena* arena, pxi32 value)
{
    ...
    pxUtf16WriteForw(result, length, 0, value);
...
{
    return pxString16CopyMemory16(arena, string.memory, string.length);
}
...
{
    return pxString16ChainMemory16(self, arena, value.memory, value.length);
}
...
{
    return pxString16IsEqualMemory16(self, value.memory, value.length);
}
...
pxString16IsEqualMemory16(PxString16 self, pxu16* memory, pxiword length)
{
    if (self.length == length)
        return pxMemoryIsEqual(self.memory, memory, length, 2);
...
{
    return pxString16BeginsWithMemory16(self, value.memory, value.length);
}
...
pxString16BeginsWithMemory16(PxString16 self, pxu16* memory, pxiword length)
{
    ...
    return pxString16IsEqualMemory16(slice, memory, length);
}

pxb8
pxString16EndsWith(PxString16 self, PxString16 value)
{
    return pxString16EndsWithMemory16(self, value.memory, value.length);
}
...
{
    return pxString16ContainsMemory16(self, value.memory, value.length);
}
...
pxString16ContainsMemory16(PxString16 self, pxu16* memory, pxiword length)
{
    ...
    for (pxiword i = 0; i < self.length - length + 1; i += 1) {
        ...
        if (pxString16IsEqualMemory16(slice, memory, length) != 0)
            result += 1, i += length - 1;
    }
...
pxString16TrimSpaces(PxString16 self)
{
    ...
    for (; start < stop; start += units) {
        ...
        units = pxUtf16ReadForw(self.memory,
    ...
    for (; start < stop; stop -= units) {
        ...
        units = pxUtf16ReadBack(self.memory,
...
{
    if (pxString16BeginsWithMemory16(self, memory, length) != 0)
...
{
    if (pxString16EndsWithMemory16(self, memory, length) != 0)
...
{
    return pxString16FindFirstMemory16(self, start, value.memory, value.length, index);
}
...
{
    return pxString16FindLastMemory16(self, start, value.memory, value.length, index);
}
...
pxString16Next(PxString16 self, pxiword index, pxiword* units, pxi32* value)
{
    ...
    pxiword step = pxUtf16ReadForw(self.memory,
...
pxString16Prev(PxString16 self, pxiword index, pxiword* units, pxi32* value)
{
    ...
    pxiword step = pxUtf16ReadBack(self.memory,
        self.length, index, value);
